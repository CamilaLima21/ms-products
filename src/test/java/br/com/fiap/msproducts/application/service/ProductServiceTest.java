...
class ProductServiceTest {
    ...
    void shouldCreateProductSuccessfully() {
        ProductDto dto = new ProductDto(0L, "Product A", "SKU123", BigDecimal.valueOf(100.0));
        Product product = new Product(0L, "Product A", "SKU123", BigDecimal.valueOf(100.0));
        ProductEntity entity = new ProductEntity();
        ProductEntity savedEntity = new ProductEntity();
        Product savedProduct = new Product(1L, "Product A", "SKU123", BigDecimal.valueOf(100.0));
        ProductDto savedDto = new ProductDto(1L, "Product A", "SKU123", BigDecimal.valueOf(100.0));

        when(repository.existsByProductSku(dto.productSku())).thenReturn(false);
    ...
    void shouldThrowExceptionWhenSkuAlreadyExists() {
        ProductDto dto = new ProductDto(0L, "Product A", "SKU123", BigDecimal.valueOf(100.0));
        when(repository.existsByProductSku("SKU123")).thenReturn(true);
    ...
    void shouldReturnAllProducts() {
        ...
        Product product1 = new Product(1L, "Product A", "SKU123", BigDecimal.valueOf(50.0));
        Product product2 = new Product(2L, "Product B", "SKU456", BigDecimal.valueOf(75.0));
        ProductDto dto1 = new ProductDto(1L, "Product A", "SKU123", BigDecimal.valueOf(50.0));
        ProductDto dto2 = new ProductDto(2L, "Product B", "SKU456", BigDecimal.valueOf(75.0));
    ...
    void shouldFindProductById() throws ProductNotFoundException {
        ProductEntity entity = new ProductEntity();
        Product product = new Product(1L, "Product A", "SKU123", BigDecimal.valueOf(50.0));
        ProductDto dto = new ProductDto(1L, "Product A", "SKU123", BigDecimal.valueOf(50.0));
    ...
    void shouldUpdateProductSuccessfully() throws ProductNotFoundException {
        ProductDto dto = new ProductDto(1L, "Updated Product", "SKU123", BigDecimal.valueOf(150.0));
        ProductEntity entity = new ProductEntity();
        Product updatedProduct = new Product(1L, "Updated Product", "SKU123", BigDecimal.valueOf(150.0));
        ProductDto updatedDto = new ProductDto(1L, "Updated Product", "SKU123", BigDecimal.valueOf(150.0));
    ...
    void shouldFindProductBySku() throws ProductNotFoundException {
        ProductEntity entity = new ProductEntity();
        Product product = new Product(1L, "Product", "SKU999", BigDecimal.valueOf(99.99));
        ProductDto dto = new ProductDto(1L, "Product", "SKU999", BigDecimal.valueOf(99.99));

        when(repository.findByProductSku("SKU999")).thenReturn(Optional.of(entity));
        when(mapper.toDomain(entity)).thenReturn(product);
        when(mapper.toDto(product)).thenReturn(dto);

        ProductDto result = service.findBySku("SKU999");

        assertEquals("SKU999", result.productSku());
    }

    @Test
    void shouldThrowWhenProductNotFoundBySku() {
        when(repository.findByProductSku("NOT_FOUND")).thenReturn(Optional.empty());
        assertThrows(ProductNotFoundException.class, () -> service.findBySku("NOT_FOUND"));
    }

    @Test
    void shouldValidateExistingSkusSuccessfully() {
        List<String> skus = List.of("SKU1", "SKU2");

        ProductEntity entity1 = new ProductEntity();
        entity1.setProductSku("SKU1");

        ProductEntity entity2 = new ProductEntity();
        entity2.setProductSku("SKU2");

        when(repository.findAllByProductSkuIn(skus)).thenReturn(List.of(entity1, entity2));

        assertDoesNotThrow(() -> service.validateSkus(skus));
    }

    @Test
    void shouldThrowWhenSomeSkusAreNotFound() {
        List<String> skus = List.of("SKU1", "SKU2", "SKU3");

        ProductEntity entity1 = new ProductEntity();
        entity1.setProductSku("SKU1");

        ProductEntity entity2 = new ProductEntity();
        entity2.setProductSku("SKU3");

        when(repository.findAllByProductSkuIn(skus)).thenReturn(List.of(entity1, entity2));

        ProductNotFoundException exception = assertThrows(ProductNotFoundException.class, () -> {
            service.validateSkus(skus);
        });

        assertTrue(exception.getMessage().contains("SKU2")); // SKU2 está ausente
    }
